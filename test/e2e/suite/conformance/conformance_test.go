...
var _ = SynchronizedBeforeSuite(
	...
	func(ctx context.Context) []byte {
		...
		credentialProviders := suite.CredentialProviders(test)
		osList := suite.OSList()

		// pick 3 random nodes from the list for conformance tests
		nodesToCreate := []suite.NodeCreate{}
		for range numberOfNodes {
			randomOSIndex := rand.Intn(len(osList))
			randomProviderIndex := rand.Intn(len(credentialProviders))
			os := osList[randomOSIndex]
			provider := credentialProviders[randomProviderIndex]
