...
func (e *E2EReport) parseJSONReport(reportPath string) (E2EResult, error) {
	...
	for _, spec := range report.SpecReports {
		...
		if artifactsPath != "" {
			failedTest.GinkgoLog = filepath.Join(artifactsPath, testGinkgoOutputLog)
			failedTest.SerialLog = filepath.Join(artifactsPath, constants.SerialOutputLogFile)
		}
		...
		if saveErr := e.saveTestLogFile(spec, failedTest.InstanceName, failedTest.Name); saveErr != nil {
			err = errors.Join(err, fmt.Errorf("saving log file: %w", saveErr))
		}
	}

	if e2eResult.ArtifactsBucketPath != "" {
		e2eResult.GinkgoLog = filepath.Join(e2eResult.ArtifactsBucketPath, testGinkgoOutputLog)
		e2eResult.SetupLog = filepath.Join(e2eResult.ArtifactsBucketPath, testSetupLogFile)
		e2eResult.CleanupLog = filepath.Join(e2eResult.ArtifactsBucketPath, testCleanupLogFile)
	}
...
func (e *E2EReport) saveTestLogFile(spec ginkgoTypes.SpecReport, instanceName, specName string) error {
