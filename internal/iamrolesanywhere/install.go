...
const (
	// DefaultSigningHelperBinPath is the path that the signing helper is installed to.
	DefaultSigningHelperBinPath = "/usr/local/bin/aws_signing_helper"
...
func Install(ctx context.Context, opts InstallOptions) error {
	if opts.BinPath == "" {
		opts.BinPath = DefaultSigningHelperBinPath
	}
...
func downloadFileTo(ctx context.Context, opts InstallOptions) error {
	...
	if err := artifact.InstallFile(opts.BinPath, signingHelper, artifactFilePerms); err != nil {
		return errors.Wrap(err, "installing aws_signing_helper")
	}
...
func Uninstall() error {
	...
	return os.RemoveAll(DefaultSigningHelperBinPath)
}
...
func Upgrade(ctx context.Context, signingHelperSrc SigningHelperSource, log *zap.Logger) error {
	...
	return artifact.Upgrade(artifactName, DefaultSigningHelperBinPath, signingHelper, artifactFilePerms, log)
}
