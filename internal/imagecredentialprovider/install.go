...
const (
	// DefaultBinPath is the path to the image-credential-provider binary.
	DefaultBinPath = "/etc/eks/image-credential-provider/ecr-credential-provider"
...
func downloadFileTo(ctx context.Context, opts InstallOptions) error {
	...
	if err := artifact.InstallFile(opts.BinPath, imageCredentialProvider, artifactFilePerms); err != nil {
		return errors.Wrap(err, "installing image-credential-provider")
	}
...
func Uninstall() error {
	return os.RemoveAll(path.Dir(DefaultBinPath))
}
...
func Upgrade(ctx context.Context, src Source, log *zap.Logger) error {
	...
	return artifact.Upgrade(artifactName, DefaultBinPath, imageCredentialProvider, artifactFilePerms, log)
}
